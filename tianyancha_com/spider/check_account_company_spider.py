...
class NormalAccountCompanySpider(object):
    ...
    def search_account_from_mongo(self):
        ...
        while True:
            try:
                try:
                    account_info = self.account_collection.find_one({'usable': 0, 'flag': {'$ne':2}})
                except Exception as e:
                    print(e)
                    account_info = None
                if account_info:
        ...
        try:
            ...
            try:
                ...
                if no_use:
                    # 提前打标记
                    # 标记为不可用，下次不再取出
                    # self.account_collection.update_one({'_id': self.account_id}, {'$set': {"usable": 1}})
                    self.flag = True
                    self.login()
                    return
            except Exception as e:
                ...
                startpart1 = '<span>天眼查为你找到</span><span class="num">'
                end0 = 0
                startpart00 = '天眼查为你找到<span class="num-company-distributed">&nbsp;'
                end00 = 0
                if html0.find(startpart0, end0) != -1 or html0.find(startpart1, end0) != -1:
                    # 标记为可用，
                    self.account_collection.update_one({'_id': self.account_id}, {'$set': {"usable": 1}})
                    self.check_count = 0
                    self.flag = True
...
if __name__=='__main__':
    ...
    db_eq = client['ip_db']
    # res = db_eq['vps_static_ip_results'].find({
    #     'flag': True,
    # })
    # for _ in res:
    #     try:
