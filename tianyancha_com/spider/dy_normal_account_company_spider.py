...
from common import redis_conn, name_results_coll, account_results_coll,update_db, local_all_com
...
class NormalAccountCompanySpider(object):
    ...
    def get_name_to_search(self, log):
        ...
        self.search_company_from_mongo(log)
        # 进行编码转换
    ...
    def search_company_from_mongo(self, log):
        '''
        从数据库查询公司名
        :return:
        '''
        ...
        while True:
            try:
                #添加新字段
                try:
                    name_info = json.loads(redis_conn.rpop('tyc_name_chengdu_lists').decode())
                    res = local_all_com['rongzi_chendu'].find_one({'_id': name_info['_id']})
                    if res.get('flag'):
                        print('已爬取')
                        continue
                except Exception as e:
                ...
                if name_info:
                    ...
                    # 人名
                    self.man_name = name_info["companyName"]
                    break
                    ...
                    one = local_all_com['rongzi_chendu'].find_one({'flag': 0})
                    if one:
                        sleep(60)
                        ...
                        if '普通用户可查看100家公司，VIP会员可查看5000家公司' in self.driver.page_source:
                            ...
                            try:
                                ...
                                if error_str == "抱歉，没有找到相关结果！":
                                    ...
                                    # )
                                    local_all_com['rongzi_chendu'].update_one({'_id': self._id}, {'$set': {'flag': 1}})
                                    self.check_count += 1
    ...
    def parse_company_pages(self, log):
        ...
        while True:
            try:
                ...
                # 获取老板数量
                try:
                    tmp = self.driver.find_element_by_xpath('//div[contains(text(),"天眼查为你找到")]/..').text
    ...
    def get_next_page(self, log):
        ...
        if data:
            ...
            # local_all_com['all_chengdu_results'].update_one({'_id': self._id}, {'$set': {"flag": 2}})
