...
import sys
sys.path.append(r"F:\projects\04_Coding\spider")
# import core as C
...
class NormalAccountCompanySpider(object):
    ...
    def __init__(self, img_path, ip, skip_count):
        ...
        self.client = pymongo.MongoClient(
            'mongodb://rwuser:48bb67d7996f327b@10.2.1.216:57017,10.2.1.218:57017')
        # 天眼查查询数据库
    ...
    def search_account_from_mongo(self):
        ...
        while True:
            try:
                try:
                    # account_info = self.account_collection.find_one({'flag':2})
                    account_info = self.account_collection.find_one()
                except Exception as e:
                    print(e)
                    account_info = None
                if account_info:
                    ...
                    log.info("数据库没有需要打码的账号...sleep 60")
                    time.sleep(60)
                    continue
            except Exception as e:
        ...
        while True:
            try:
                ...
                try:
                    ...
                    name_info = self.name_results.find_one()
                except Exception as e:
                    print(e)
                    name_info = None
                if name_info:
                    ...
                    one = self.name_results.find_one({"name_num": {'$exists': False}})
                    if one:
                        time.sleep(120)
                        log.info('缓存中暂时没有数据,120s后重试...')
                        continue
                    else:
    ...
    def login(self):
        ...
        while self.flag:
            try:
                ...
                # 账号,密码
                str = """document.querySelector(".mobile_box .contactphone").value = '{}';document.querySelector(".mobile_box .contactword").value = '{}';""".format(
                    self.account_num, self.password)
    ...
    def handle_question(self):
        ...
        try:
            ...
            if home_pages:
                #提前打标记
                # self.account_collection.update_one({'_id': self.account_id}, {'$set': {"flag": 1}})
                self.flag=False
                return
        except Exception as e:
            ...
            try:
                ...
                if no_use:
                    # 提前打标记
                    # 标记为不可用，下次不再取出
                    self.account_collection.update_one({'_id': self.account_id}, {'$set': {"usable": 1, 'flag': 3}})
                    return
            except Exception as e:
                # 获取列表页所有详情urls
                try:
                ...
                #都找不到说明没有进入列表页
                    ...
                    # 如果出现检索条件过大，或者账号暂时不可用
                        ...
                        if '普通用户可查看100家公司，VIP会员可查看5000家公司' in self.driver.page_source:
                            # 提前打标记
                            self.account_collection.update_one({'_id': self.account_id}, {'$set': {"flag": 0}})
                            # self.search_name_from_mongo()
                            # self.get_name_to_search()
                        else:
                            try:
    ...
    def parse_company_pages(self):
        ...
        while True:
            try:
                ...
                startpart4 = '<span>天眼查为你找到</span><span class="num">'
                endpart3 = '&nbsp;</span>家公司'
                end3 = 0
                """--------------------"""
                ...
                if list_html.find(startpart2, end2) != -1:
                    ...
                    log.info("该名字老板数量:{}".format(self.name_num))
                else:
                    # log.info("[INFO]: 老板数量获取失败")
                    self.name_num = 0
                ...
                if list_html.find(startpart3, end3) != -1:
                    ...
                    if '+' in company_num_str:
                        self.db.name_results.update_one(
                            {'_id': self._id}, {'$set': {"name_num": self.name_num, "company_numm": 0, 'flag': 2}}
                        )
                        return
                    else:
                        self.company_num = int(company_num_str)
                    log.info("该名字公司数量:{}".format(self.company_num))
                    # 存储更新数据库字段
                    self.update_name_db()
                    # 判断类型
                    self.judge_num(self.company_num)
                elif list_html.find(startpart4, end3) != -1:
                    start = list_html.find(startpart4, end3) + len(startpart4)
                    end = list_html.find(endpart3, start)
                    company_num_str = list_html[int(start):int(end)]
                    if '+' in company_num_str:
                        self.db.name_results.update_one(
    ...
    def save_data(self, dic):
        ...
        '''
        self.total += 1
        with open('fix_count.txt', 'w') as f:
            f.write('入库总数：{}'.format(self.total))
        try:
            import time
...
if __name__=='__main__':
    ...
    file_name = "{}check_account_company_{}.log".format(real_path,time.strftime("%Y-%m-%d",time.localtime()))
    log.basicConfig(
        level=log.INFO,
    ...
    ip = ip_results_coll.find_one({'flag': True}).get('ip')
    start(img_path, ip, 1)
