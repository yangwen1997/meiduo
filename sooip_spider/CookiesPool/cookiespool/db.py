...
class CookiesRedisClient(RedisClient):
    ...
    def get_key(self, raw_key):
        return raw_key.decode('utf8').split(':')[-1]

    def random_add_account(self):
        try:
            key = random.choice(self.keys())
            account = self.get_key(key)
            cookies = eval(self._db.get(key))
            return dict(account=account, cookies=cookies)
        except:
            raise GetRandomCookieError
...
if __name__ == '__main__':
    ...
    """
    # 测试
    conn = AccountRedisClient(name='WeiMao')
    conn.delete('17191474579')
